...
export function TaskForm({ mode, taskId, initial }: TaskFormProps) {
  ...
  const [form, setForm] = useState(initial || emptyInitial);
  ...
  useEffect(() => {
    void Promise.all([
      ...
      fetch("/api/tasks/auth/me").then((r) => r.json()),
    ]).then(([teamData, fieldsData, meData]) => {
      setTeam(teamData.team || []);
      setFields(fieldsData.fields || []);
      const uid = meData.user?.id || "";
      setCurrentUserId(uid);
      if (mode === "create") {
        setForm((prev) => (prev.assigneeId ? prev : { ...prev, assigneeId: uid }));
      }
    });
  }, [mode]);
